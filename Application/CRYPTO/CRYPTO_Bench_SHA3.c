...
File        : CRYPTO_Bench_SHA3.c
Purpose     : Benchmark SHA-3 implementation.
...
static void _HashBenchmark(const char *sAlgorithm, const CRYPTO_HASH_API *pAPI) {
  CRYPTO_SHA3_256_CONTEXT C; // big enough for most things...
  U64                     T0;
  U64                     OneSecond;
  unsigned                n;
  //
...
void MainTask(void) {
  ...
  SEGGER_SYS_IO_Printf("SHA-3 Benchmark compiled " __DATE__ " " __TIME__ "\n\n");
  //
  ...
  SEGGER_SYS_IO_Printf("Config:   CRYPTO_CONFIG_SHA3_OPTIMIZE    = %d\n\n", CRYPTO_CONFIG_SHA3_OPTIMIZE);
  //
  SEGGER_SYS_IO_Printf("+--------------+-----------+\n");
  SEGGER_SYS_IO_Printf("| Algorithm    | Hash MB/s |\n");
  SEGGER_SYS_IO_Printf("+--------------+-----------+\n");
  //
  _HashBenchmark("SHA-3", &CRYPTO_HASH_SHA3_256_SW);
  CRYPTO_SHA3_256_QueryInstall(&pHWAPI, &pSWAPI);
  if (pHWAPI && pHWAPI != &CRYPTO_HASH_SHA3_256_SW) {
    _HashBenchmark("SHA-3 (HW)", pHWAPI);
  }
  SEGGER_SYS_IO_Printf("+--------------+-----------+\n");
  //
  SEGGER_SYS_IO_Printf("\nBenchmark complete\n");
