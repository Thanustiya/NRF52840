...
#ifndef   USE_PREALLOCATION
  #define USE_PREALLOCATION     1                     // Configures if the file preallocation has to be used.
#endif

#if FS_SUPPORT_JOURNAL
  #ifndef   JOURNAL_SIZE
    #define JOURNAL_SIZE        0                     // Size of the Journaling file in bytes.
  #endif
#endif
...
static double _GetAverage(int Index) {
  ...
  v = (double)pResult->Av;
  if ((U32)v == 0u) {
    v = 1.0;                    // Make sure that we display the maximum speed if the timer resolution is too low.
  }
  v = (double)1000.0 / v;
...
void MainTask(void) {
  ...
  if (r == 0) {
    ...
    // Enable Journaling.
    //
    FS_JOURNAL_CreateEx(VOLUME_NAME, JOURNAL_SIZE, 1);
#endif // JOURNAL_SIZE
#else
    //
