...
static const char * _GetPath(const char * sPath) {
  if (strchr(sPath, (int)FS_VOLUME_DELIMITER) != NULL) {
    sPath = "";
  } else {
    sPath = _acCurrentDir;
  }
...
static const char * _GetVolumeName(const char * s) {
  if (strchr(s, (int)FS_VOLUME_DELIMITER) == NULL) {          // Check if s points to a volume name.
    s = _acCurrentVolume;
  }
  return s;
}
...
static void _UpdateCurrentDir(const char * s) {
  ...
  sDelimiter = strrchr(_acCurrentDir, (int)FS_VOLUME_DELIMITER);
  if (sDelimiter != NULL) {
...
*
*  Function description
*    Displays the specified data in human-readable format.
*
*  Parameters
*    Off          Byte offset of the first data.
*    pData        [IN] Data to be displayed.
*    NumBytes     Number of bytes to be displayed.
*    MaxNumLines  Maximum number to be displayed at once.
*
*  Return value
*    ==0    All data displayed.
*    !=0    Abort requested by used.
*
*  Additional information
*    If MaxNumLines is different than 0 then the function asks the user
*    for input each time this number of lines are displayed. The user
*    has the option to display the next data, to display all the remaining
*    data or to cancel the operation.
*/
static int _ShowData(U32 Off, const U8 * pData, unsigned NumBytes, unsigned MaxNumLines) {
  unsigned i;
  char     ac[16];
  U8       Data;
  ...
  while (NumBytes > 0u) {
    ...
    for (i = 0; i < NumBytesPerLine; i++) {
      ...
      if ((Data >= 0x20u) && (Data < 0x80u)) {
        SEGGER_snprintf(ac, (int)sizeof(ac), "%c", Data);
        _Log(ac);
      } else {
        _Log(".");
      }
    ...
    if (MaxNumLines != 0u) {
      ...
      if (NumLines >= MaxNumLines) {
        _Log("File larger than ");
        SEGGER_snprintf(ac, (int)sizeof(ac), "%u", MaxNumLines);
        _Log(ac);
        _Log(" line(s).\n");
        _Log("Do you want to continue? <[a]lways | [n]ext | [c]ancel>\n");
        ac[0] = 'a';
...
static int _IsVolumeName(const char * sVolumeName) {
  ...
  if (NumBytes > 0u) {
    if (sVolumeName[NumBytes - 1u] == FS_VOLUME_DELIMITER) {
...
static int _ExecChangeDir(const char * s) {
  ...
  if (strchr(_ac, (int)FS_VOLUME_DELIMITER) != NULL) {
    sPath = _ac;
...
static int _ExecMakeFile(const char* s) {
  ...
  if (r == 0) {
    ...
    if (DoFill != 0) {
      ...
      //
      (void)FS_FSeek(pFile, (I32)NumBytesToWrite, FS_SEEK_SET);
...
static int _ExecTypeHex(const char * s) {
  ...
  //
  // Parse the maximum number of lines to display (optional)
  //
  MaxNumLines = LINES_PER_PAGE;
  r = _ParseString(&s, _ac, sizeof(_ac));
  if (r == 0) {
    if (strcmp("/P", _ac) == 0) {
      (void)_ParseDec(&s, &MaxNumLines);
    }
  }
  //
  // Execute the operation.
  ...
  if (r == 0) {
    ...
    while (FS_FEof(pFile) == 0) {
      ...
      Result = _ShowData(Off, (const U8 *)_aBuffer, NumBytes, MaxNumLines);
      if (Result != 0) {
        break;
      }
      Off += NumBytes;
...
static int _ExecDiskInfo(const char * s) {
  ...
  if (r == 0) {
    ...
    BytesPerSector    = (U32)DiskInfo.BytesPerSector;
    SectorsPerCluster = (U32)DiskInfo.SectorsPerCluster | ((U32)DiskInfo.SectorsPerClusterHigh << 16);
...
#if (FS_SUPPORT_FAT != 0) || (FS_SUPPORT_EXFAT != 0)
...
#endif // (FS_SUPPORT_FAT != 0) || (FS_SUPPORT_EXFAT != 0)
...
*       _ExecFormatSDEx
*/
static int _ExecFormatSDEx(const char * s) {
  int          r;
  const char * sVolumeName;
  ...
  _LogV(sVolumeName);
  _LogV("\"...");
  r = FS_EXFAT_FormatSD(sVolumeName);
  _LogOK(r);
  if (r == 0) {
    _ChangeToRootDir();
  }
...
static int _ExecMeasureWriteSpeedPA(const char * s) {
  ...
  FS_SetFilePos(pFile, (I32)FileSize, FS_SEEK_SET);
  r = FS_SetEndOfFile(pFile);
  if (r != 0) {
    (void)FS_FClose(pFile);
...
#endif // (FS_SUPPORT_FAT != 0) || (FS_SUPPORT_EXFAT != 0)
#if FS_SUPPORT_EXFAT
  {_ExecFormatSDEx,           "fmtsdex",    "FormatSDEx",         "Performs a high-level format of volume (SD specification)",      "[<VolumeName>]"                                                     },
#endif // FS_SUPPORT_EXFAT
#if (FS_OS_LOCKING != 0)
