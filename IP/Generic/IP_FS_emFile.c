...
static int _ConvertPath(const char * sFilename, char * sOutFilename, U32 BufferSize) {
  ...
  if (_sVolumeName) {
    ...
    // Ignore the root directory. This is the case if volumes
    ...
    // have to restore the colons to build the real FS volume again.
    //
...
static int _ReadAt(void * hFile, void *pDest, U32 Pos, U32 NumBytes) {
  ...
  // Set read position
  //
  r = FS_FSeek((FS_FILE*) hFile, Pos, FS_SEEK_SET);
  if (r != 0) {
#if DEBUG
    ErrCode = FS_FError((FS_FILE*) hFile);
    printf("FS_FSeek() failed, error code %s\n", FS_ErrorNo2Text(ErrCode));
#endif
    return -1;
  }
  //
  // Read from the file.
  //
  r = FS_Read((FS_FILE*) hFile, pDest, NumBytes);
  if ((U32)r != NumBytes) {
    ErrCode = FS_FError((FS_FILE*) hFile);
    if (ErrCode != 0) {
      if (ErrCode != FS_ERRCODE_EOF) { // EOF is not an actual error.
#if DEBUG
        printf("FS_Read() failed, error code %s\n", FS_ErrorNo2Text(ErrCode));
#endif
        return -1;
      }
    }
...
static long _GetLen(void * hFile) {
  ...
  _InitIfRequired();
  r = FS_GetFileSize((FS_FILE*) hFile);
  if (r == 0xFFFFFFFF) {
    return -1;
  }
  return r;
...
*       _ForEachDirEntryEx()
*/
static int _ForEachDirEntryEx(void * pContext, const char * sDir, void (*pf) (void * pContext, void * pFileEntry), int ShowHidden) {
  FS_FIND_DATA fd;
  ...
  if (acDirname[0] != 0) {
    ...
    if (Attrib != 0xFF) {
      if ((Attrib & FS_ATTR_HIDDEN) != 0) {
        if (ShowHidden == 0) {
          return 0;  // Access to hidden folder not permitted.
        }
      }
    }
  }
  if (_sVolumeName || (acDirname[0] != 0)) {  // No multi volume support enabled OR not root directory
    ...
    NumVolumes = FS_GetNumVolumes();
    for (iVolume = 0; iVolume < NumVolumes; ++iVolume) {
      r = FS_GetVolumeName(iVolume, acVolumeName, sizeof(acVolumeName));
      if (r < 0) {
        return -1;
      }
...
*       _ForEachDirEntry()
*/
static int _ForEachDirEntry(void * pContext, const char * sDir, void (*pf) (void * pContext, void * pFileEntry)) {
  return _ForEachDirEntryEx(pContext, sDir, pf, 1);
}
...
*/
static int _ForEachDirEntry_NoHidden(void * pContext, const char * sDir, void (*pf) (void * pContext, void * pFileEntry)) {
  return _ForEachDirEntryEx(pContext, sDir, pf, 0);
}
...
*
*  Obsolete implementation for _ForEachDirEntry(). Does not use the return value of _ForEachDirEntryEx().
*/
static void _ForEachDirEntry_Obsolete(void * pContext, const char * sDir, void (*pf) (void * pContext, void * pFileEntry)) {
  (void)_ForEachDirEntryEx(pContext, sDir, pf, 1);
}

/*********************************************************************
*
*       _ForEachDirEntry_NoHidden_Obsolete()
*
*  Obsolete implementation for _ForEachDirEntry_NoHidden(). Does not use the return value of _ForEachDirEntryEx().
*/
static void _ForEachDirEntry_NoHidden_Obsolete(void * pContext, const char * sDir, void (*pf) (void * pContext, void * pFileEntry)) {
  (void)_ForEachDirEntryEx(pContext, sDir, pf, 0);
}
...
const IP_FS_API IP_FS_FS = {
  ...
  //
  _ForEachDirEntry_Obsolete,          // pfForEachDirEntry_Obsolete
...
const IP_FS_API IP_FS_FS_AllowHiddenAccess = {
  ...
  // Directory query operations.
  //
  _ForEachDirEntry_NoHidden_Obsolete, // pfForEachDirEntry_Obsolete
  _GetDirEntryFilename,               // pfGetDirEntryFileName
  _GetDirEntryFileSize,               // pfGetDirEntryFileSize
  _GetDirEntryFileTime,               // pfGetDirEntryFileTime
  _GetDirEntryAttributes,             // pfGetDirEntryAttributes
