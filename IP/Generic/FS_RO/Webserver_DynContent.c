...
#ifndef   WEBS_USE_SAMPLE_EMUSB_WEB
  #define WEBS_USE_SAMPLE_EMUSB_WEB  0
#endif
...
#if (WEBS_USE_SAMPLE_2018 == 0 && WEBS_USE_SAMPLE_EMUSB_WEB == 0 && WEBS_USE_SAMPLE_TEMPLATING == 0)
...
static void _cbOnProgressNotify(WEBS_PROGRESS_INFO* pInfo) {
  ...
  if        (Status == WEBS_PROGRESS_STATUS_END) {
    ...
    if ((WEBS_MEMCMP(sURI + Len - 4, ".htm" , sizeof(".htm"))  == 0) ||
        (WEBS_MEMCMP(sURI + Len - 5, ".html", sizeof(".html")) == 0)) {
...
#if defined(_WIN32) || defined(__linux__) || defined(__APPLE__) || defined(USB_WEBSERVER)
  IP_WEBS_SendString(pOutput, "<H2>This sample runs on a Windows host.<br>No statistics available.</H2>");
#elif defined(USE_OS)
  #if (USE_OS == OS_UCOSIII)
    IP_WEBS_SendString(pOutput, "<H2>This sample runs on a host with uC-OS-III.<br>No statistics available.</H2>");
  #endif
...
/*********************************************************************
*
*       _callback_VFILE_SSENet
*
*  Function description
*    Sends some network information to the Web browser every 500 ms.
*/
static void _callback_VFILE_SSENet(WEBS_OUTPUT* pOutput, const char* sParameters) {
  int r;
#ifdef USE_OS
  #if (USE_OS == OS_UCOSIII)
    OS_ERR Os_Err;
...
static void _callback_VFILE_GetDetails(WEBS_OUTPUT* pOutput, const char* sParameters) {
  ...
  } else if (WEBS_STRCMP(acPara, "IP") == 0) {
  ...
  } else if (WEBS_STRCMP(acPara, "FS") == 0) {
  ...
  } else if (WEBS_STRCMP(acPara, "GUI") == 0) {
  ...
  } else if (WEBS_STRCMP(acPara, "BTL") == 0) {
  ...
  } else if (WEBS_STRCMP(acPara, "USBD") == 0) {
  ...
  } else if (WEBS_STRCMP(acPara, "USBH") == 0) {
...
static const WEBS_VFILES _aVFiles[]  = {
  ...
  {"/SSENet.cgi",         _callback_VFILE_SSENet            },   // Called from SSE_IP.htm
...
//  {"GetIFaceInfos"      , _callback_ExecGetIFaceInfo      },
  ...
  {"AssignIPType"       , _callback_IPv4ConfigType        },
#endif // INCLUDE_IP_CONFIG_SAMPLE
...
static int _UPnP_CheckVFile(const char * sFileName, unsigned * pIndex) {
  ...
  // Generated VFiles
  //
  if (WEBS_STRCMP(sFileName, "/upnp.xml") == 0) {
    return 1;
  }
  //
  // Static VFiles
  ...
  for (i = 0; i < SEGGER_COUNTOF(_VFileList); i++) {
    if (WEBS_STRCMP(sFileName, _VFileList[i].sFileName) == 0) {
      *pIndex = i;
      return 1;
    }
  }
...
#endif // (WEBS_USE_SAMPLE_2018 == 0 && WEBS_USE_SAMPLE_EMUSB_WEB == 0)
