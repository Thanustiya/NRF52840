...
static void _DLIST_RemoveDelete(TCP_WAIT* pTCPWait) {
  ...
  if (pTCPWait->pPrev) {
    ...
    _IP_OS_Static.pTCPWait = pTCPWait->pNext;
  }
  if (pTCPWait->pNext) {
...
static void _IP_OS_cbOnTerminateTask(OS_CONST_PTR OS_TASK* pTask) {
  ...
  for (pTCPWait = _IP_OS_Static.pTCPWait; pTCPWait; pTCPWait = pTCPWait->pNext) {
...
static void _IP_OS_cbOnExit(void) {
  ...
  OS_RemoveTerminateHook(&_IP_OS_Static.OnTerminateTaskHook);
  IP_OS_pIPTask    = NULL;
  IP_MEMSET(&_IP_OS_Static, 0, sizeof(_IP_OS_Static));
}
#endif
...
const IP_OS_API* IP_OS_Init(void) {
  if (_IP_OS_Static.IsInited == 0u) {
...
#endif
#if (OS_VERSION >= 38606)  // OS_AddOnTerminateHook() is supported since embOS v3.86f .
    OS_AddOnTerminateHook(&_IP_OS_Static.OnTerminateTaskHook, _IP_OS_cbOnTerminateTask);  // Has been renamed to OS_AddTerminateHook() with embOS v4.30 using a compatibility macro.
#endif
#if (IP_ALLOW_DEINIT && (OS_VERSION >= 43000))
    //
    ...
    // twice due to _IP_OS_Static.IsInited != 0u .
    //
    IP_AddOnExitHandler(&_IP_OS_Static.OnExitHook, _IP_OS_cbOnExit);
#endif
    _IP_OS_Static.IsInited = 1u;
  }
  return &_IP_OS_Api;
}
...
unsigned IP_OS_WaitNetEventTimed(unsigned Timeout) {
  ...
  if (Prio > _IP_OS_Static.MaxNonRxTaskPrio) {
    _IP_OS_Static.MaxNonRxTaskPrio = Prio;
  }
  //
...
#endif
  TCPWait.pNext          = _IP_OS_Static.pTCPWait;
  _IP_OS_Static.pTCPWait       = &TCPWait;
...
#if (OS_VERSION >= 36000)
  static OS_TICK_HOOK _IP_OS_TickHook;
