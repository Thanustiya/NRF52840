...
#endif
  U8                     POSTRecvd;                     // Marker for post-redirect-get pattern used in _cbBeforeContentOutput().
} USER_CONTEXT;
...
//
static WEBS_REQUEST_NOTIFY_HOOK     _RequestNotifyHook;
...
//lint -esym(534, IP_WEBS_SendHeaderEx, IP_WEBS_SendHeader, IP_WEBS_METHOD_CopyData, WEBS_SNPRINTF, getsockname, IP_SOCK_getsockname)
//
...
static void _cbOnProgressNotify(WEBS_PROGRESS_INFO* pInfo) {
  ...
  //
  if (Status == WEBS_PROGRESS_STATUS_BEGIN) {
    //
    // A request is about to be processed.
    // Context sensitive allocations can easily be done here.
    //
#if defined(_WIN32) || defined(__linux__) || defined(__APPLE__) || defined(USB_WEBSERVER)
  ...
  //
  if        (Status == WEBS_PROGRESS_STATUS_END) {
    //
  ...
  } else if (Status == WEBS_PROGRESS_STATUS_USER_AUTH) {
    //
    // A user authorization event has happened.
    //
    if (pInfo->Data.UserAuth.Event == WEBS_USER_AUTH_EVENT_SUCCESS) {
      //
      // A user has been successfully authorized for a requested and protected resource.
      // More information like the username can be extracted from the WEBS_ACCESS_CONTROL
      // item at "pInfo->Data.UserAuth.pAccess". As these might be specific to the
      // authorization method used (e.g. Basic or Digest), the sample only outputs generic
      // information available in all cases.
      //
      WEBS_LOG(("WebS: Authorized for protected path \"%s\"", pInfo->Data.UserAuth.pAccess->sPath));
    }
#if 0
  } else if (Status == WEBS_PROGRESS_STATUS_METHOD_URI_VER_PARSED) {
    //
...
#endif
#if 0
  } else if (Status == WEBS_PROGRESS_STATUS_HEADER_PARSED) {
    //
...
/*********************************************************************
*
*       _cbRequestNotifyHook
*
*  Function description
*    Callback that is called by webserver when an URI or METHOD request is issued to the webserver.
*    We usually use this callback to know to which page this connection is connected, so we can later on call the correct CGI callbacks for CGI replacement etc.
*    When the output of the page is parsed by our webserver.
*
*  Notes
*    (1) This function is also called for all files referenced by a page. As we do not know if we are called for a HTML page or another file,
*        we do not perform any error handling in case we do not find a matching page for the given URI
*/
static void _cbRequestNotifyHook(WEBS_REQUEST_NOTIFY_INFO* pInfo) {
  USER_CONTEXT* pUserContext;

  pUserContext = (USER_CONTEXT*)IP_WEBS_RetrieveUserContext(pInfo->pOutput);
  if (pUserContext) {
    pUserContext->POSTRecvd = (pInfo->Method == METHOD_POST) ? 1 : 0;
  }
}
...
*
*  Return value
*    0: O.K., send content as usual.
*    1: Suppress output of further content.
*
*  Additional information
...
static int _cbBeforeContentOutput(WEBS_OUTPUT* pOutput) {
  ...
  if (pUserContext != NULL) {         // Check that the memory allocation succeeded, otherwise it makes no sense to try to create/update a cookie.
    ...
    //
    (void)WEBS_SNPRINTF(&pUserContext->acCookie[0], sizeof(pUserContext->acCookie), "Set-Cookie: pagecnt=%u; Max-Age=600; HttpOnly; SameSite=strict\r\n", pUserContext->NumPagesVisited);
    IP_WEBS_HEADER_SetCustomFields(pOutput, SEGGER_PTR2PTR(const char, &pUserContext->acCookie[0]));
    //
    // The redirect via 303 to the same page prevents "form resubmission"
    // When the user presses F5 after submitting a post the user will normally see a warning
    // by the browser "Confirm Form Resubmission".
    // In most cases it is not desired for a web application to allow form resubmission,
    // this post-redirect-get pattern prevents it.
    //
    if (pUserContext->POSTRecvd) {
      if (WEBS_STRCMP(sURI, "/Upload.cgi") != 0) { // Do not do this for the file upload, as it would prevent upload result evaluation.
        sURI = IP_WEBS_GetURI(pOutput, 0);
        IP_WEBS_SendLocationHeader(pOutput, sURI, "303 See Other");
        pUserContext->POSTRecvd = 0;
        Result = 1;  // No need to send page data following a POST + REDIRECT, client will request page data with a consecutive GET
      }
    }
  }
  return Result;
...
static void _callback_ExecCounter(WEBS_OUTPUT* pOutput, const char* sParameters, const char* sValue ) {
  ...
  if (_ConfigCalled == 0u) {
    IP_WEBS_SendString(pOutput, "<h1><font color=\"#FF0000\">Please call IP_WEBS_X_SampleConfig()</font></h1>");
  } else {
    _Cnt++;
...
static int _GetOSShortInfo(WEBS_OUTPUT* pOutput, unsigned UseSSE) {
  ...
  IP_WEBS_SendString(pOutput, "<H2>This sample runs on PC.<br>Network statistics are only available on an embedded target.</H2>");
#elif defined(USE_OS)
  #if (USE_OS == OS_UCOSIII)
  if (UseSSE == 1) {
    IP_WEBS_SendString(pOutput, "event: LOG_OSOverview\n");     // Write name of the Javascript event in the output buffer.
  ...
  IP_WEBS_SendString(pOutput, "<H2>This sample runs on a host with uC-OS-III.<br>No statistics available.</H2>");
  #endif
#else
  OS_TASK* pTask;
...
static void _GetIPAddrInfo(WEBS_OUTPUT* pOutput) {
  ...
  WEBS_SNPRINTF(acBuffer, sizeof(acBuffer), "%n", &IPv6Addr.Union.aU8[0]);
  IP_WEBS_SendString(pOutput, "<div class=\"row\" id=\"LOG_AddrInfo\">");
  IP_WEBS_SendString(pOutput, "<div class=\"col-sm-12\">");
...
#elif defined(USE_OS)
  #if (USE_OS == OS_UCOSIII)
    IP_WEBS_SendString(pOutput, "<H2>This sample runs on a host with uC-OS-III.<br>No network statistics available.</H2>");
  #endif
#else
    ...
    if (r == 0) {
      ...
      if (Info.LocalAddr.Union.aU32[0] == 0) {
        ...
        if (Info.AddrFam == AF_INET) {
          ...
          WEBS_SNPRINTF(acBuffer, sizeof(acBuffer), "%n", &Info.LocalAddr.Union.aU8[0]);
          IP_WEBS_SendString(pOutput, acBuffer);
        }
      }
      IP_WEBS_SendString(pOutput, "<br>");
      ...
      if (Info.ForeignAddr.Union.aU32[0] == 0) {
        ...
        if (Info.AddrFam == AF_INET) {
          ...
          WEBS_SNPRINTF(acBuffer, sizeof(acBuffer), "%n", &Info.ForeignAddr.Union.aU8[0]);
          IP_WEBS_SendString(pOutput, acBuffer);
        }
#else
    r = IP_INFO_GetConnectionInfo(hConnection[i], &Info);
...
static void _callback_ExecGetDropdown(WEBS_OUTPUT* pOutput, const char* sPara, const char* sValue) {
  ...
  IP_WEBS_SendString(pOutput, "<a class=\"dropdown-item\" href=\"/conf/Authen.htm\">Digest authentication</a>");
#else
...
/*********************************************************************
*
*       _callback_VFILE_Send
*/
static void _callback_VFILE_Send(WEBS_OUTPUT* pOutput, const char* sParameters) {
  int  r;
  char ac[64];
  ...
  if (r == 0) {
    ...
    IP_WEBS_SendString(pOutput, "To modifiy this page, open the file <keyword>Webserver_DynContent.c</keyword> and modify the function <keyword>_callback_VFILE_Send()</keyword>.");
    IP_WEBS_SendString(pOutput, "</div>");
  } else {
...
/*********************************************************************
*
*       _callback_VFILE_UploadFile
*/
static void _callback_VFILE_UploadFile(WEBS_OUTPUT* pOutput, const char* sParameters) {
...
/*********************************************************************
*
*       _callback_VFILE_SSETime
*
*  Function description
*    Sends the system time to the Web browser every 500 ms.
*/
static void _callback_VFILE_SSETime(WEBS_OUTPUT* pOutput, const char* sParameters) {
  int r;
#ifdef USE_OS
  #if (USE_OS == OS_UCOSIII)
  OS_ERR Os_Err;
  ...
  #ifdef USE_OS
    #if (USE_OS == OS_UCOSIII)
      OSTimeDlyHMSM(0,0,0,500,OS_OPT_TIME_DLY,&Os_Err);
    #endif
  #else
      OS_Delay(500);
  #endif
...
/*********************************************************************
*
*       _callback_VFILE_SSEembOS
*
*  Function description
*    Sends some OS information to the Web browser every 500 ms.
*/
static void _callback_VFILE_SSEembOS(WEBS_OUTPUT* pOutput, const char* sParameters) {
  int r;
#ifdef USE_OS
  #if (USE_OS == OS_UCOSIII)
  OS_ERR Os_Err;
...
/*********************************************************************
*
*       _callback_VFILE_SSEConnCnt
*
*  Function description
*    Sends some network information to the Web browser every 500 ms.
*/
static void _callback_VFILE_SSEConnCnt(WEBS_OUTPUT* pOutput, const char* sParameters) {
  int r;
#ifdef USE_OS
  #if (USE_OS == OS_UCOSIII)
  OS_ERR Os_Err;
...
/*********************************************************************
*
*       _callback_VFILE_SSEGetConnInfo
*
*  Function description
*    Sends some network information to the Web browser every 500 ms.
*/
static void _callback_VFILE_SSEGetConnInfo(WEBS_OUTPUT* pOutput, const char* sParameters) {
  int r;
#ifdef USE_OS
  #if (USE_OS == OS_UCOSIII)
  OS_ERR Os_Err;
...
/*********************************************************************
*
*       _callback_VFILE_SSEMaintenanceInfo
*
*  Function description
*    Sends system time and SSE update counter.
*/
static void _callback_VFILE_SSEMaintenanceInfo(WEBS_OUTPUT* pOutput, const char* sParameters) {
  int r;
#ifdef USE_OS
  #if (USE_OS == OS_UCOSIII)
  OS_ERR Os_Err;
...
/*********************************************************************
*
*       _callback_VFILE_GetDetails
*
*  Function description
*    Sends the selected product information to the browser.
*/
static void _callback_VFILE_GetDetails(WEBS_OUTPUT* pOutput, const char* sParameters) {
  char acPara[10];
...
/*********************************************************************
*
*       _callback_VFILE_GetData
*
*  Function description
*    Generates a comma separated list of values used to draw the
*    graph of stock prices.
*/
static void _callback_VFILE_GetData(WEBS_OUTPUT* pOutput, const char* sParameters) {
  char  acPara[10];
  char  ac[160];
  char* pStr;
  int   NumBytes;
  ...
  for(i = 0; i < Size; i++) {
    ...
    NumBytes      = WEBS_SNPRINTF(pStr, NumBytesFree, "%d,", v);
...
static char* _itoa(int Value, char* pStr, int radix) {
  ...
  WEBS_SNPRINTF(pStr, 10, "%d", Value);  // Just assume that we have enough space for up to 10 digits in buffer.
  return pStr;
}
...
/*********************************************************************
*
*       _callback_VFILE_SSEGetShareTable
*
*  Function description
*    Sends the stock price table
*/
static void _callback_VFILE_SSEGetShareTable(WEBS_OUTPUT* pOutput, const char* sParameters) {
  int r;
  ...
  if (sValue != NULL) {
    ...
    if (v == 1) {
      if (_DHCPOnOff == 0) {                      // Is DHCP client disabled ?
        (void)IP_DHCPC_ConfigAlwaysStartInit(0, 1);
...
*
*  Function description
*    Callback notified about incorrect credentials for an authentication.
*
*  Parameters
*    pInfo: Structure of type WEBS_AUTH_FAILED_INFO that contains more
*           information.
*/
static void _pfAuthFailed(WEBS_AUTH_FAILED_INFO* pInfo) {
  WEBS_LOG(("Authentication failed when attempting to login to %s. Doing a 5s delay.", pInfo->pPath));

  if (pInfo->acUserPass != NULL) {
    //
    // For Basic Authentication, show the credentials the user tried to login with.
    //
    WEBS_LOG(("Used credentials: %s", pInfo->acUserPass));
  }
  //
  // Do a 5s delay.
  //
  ...
  #ifdef USE_OS
    #if (USE_OS == OS_UCOSIII)
  OSTimeDlyHMSM(0,0,0,5000,OS_OPT_TIME_DLY,&Os_Err);
    #endif
  #else
  OS_Delay(5000);
  #endif
...
#if INCLUDE_SHARE_SAMPLE
  {"/GetData.cgi",           _callback_VFILE_GetData             },   // Called from Shares.htm
  {"/GetShareTable.cgi",     _callback_VFILE_SSEGetShareTable    },   // Called from Shares.htm
#endif
#if INCLUDE_PRESENTATION_SAMPLE
  {"/GetDetails.cgi",        _callback_VFILE_GetDetails          },   // Called from Presentation.htm
#endif
  {"/MaintenanceInfo.cgi",   _callback_VFILE_SSEMaintenanceInfo  },   // Called from various Web pages.
  {"/Send.cgi",              _callback_VFILE_Send                },   // Called from VirtFile.htm
  {"/SSETime.cgi" ,          _callback_VFILE_SSETime             },   // Called from SSE_Time.htm
  {"/SSEembOS.cgi",          _callback_VFILE_SSEembOS            },   // Called from SSE_OS.htm
  {"/SSEGetConnInfo.cgi",    _callback_VFILE_SSEGetConnInfo      },   // Called from index.htm
  {"/SSEConnCnt.cgi",        _callback_VFILE_SSEConnCnt          },   // Called from index.htm
//  {"/Upload.cgi",            _callback_VFILE_UploadFile          },   // Called from Upl.htm and Upl_AJAX.htm
  { NULL, NULL }
};
...
#if !defined(_WIN32) && !defined(__linux__) && !defined(__APPLE__)  // UPnP currently can not be used standalone.
...
static const char * _UPnP_GetModelNumber(unsigned IFaceId, char * pBuffer, unsigned NumBytes) {
  ...
  if (NumBytes <= 12) {
    *pBuffer = '\0';  // Just to be on the safe if the buffer is too small
    ...
    WEBS_SNPRINTF(pBuffer, NumBytes, "%02X%02X%02X%02X%02X%02X", aHWAddr[0], aHWAddr[1], aHWAddr[2], aHWAddr[3], aHWAddr[4], aHWAddr[5]);
  }
  return pBuffer;
}
...
#define UDN_PREFIX "uuid:95232DE0-3AF7-11E2-81C1-"
  ...
  *pBuffer = '\0';  // Just to be on the safe if the buffer is too small
  ...
  if (NumBytes > 12) {
    IP_GetHWAddr(IFaceId, aHWAddr, sizeof(aHWAddr));
    WEBS_SNPRINTF(p, NumBytes, "%02X%02X%02X%02X%02X%02X", aHWAddr[0], aHWAddr[1], aHWAddr[2], aHWAddr[3], aHWAddr[4], aHWAddr[5]);
  }
  return pBuffer;
}
...
static const char * _UPnP_GetFriendlyName(unsigned IFaceId, char * pBuffer, unsigned NumBytes) {
  ...
  *pBuffer = '\0';  // Just to be on the safe if the buffer is too small
  ...
  WEBS_SNPRINTF(p, NumBytes, " IFace #%lu", IFaceId);
  return pBuffer;
}
...
void IP_WEBS_X_SampleConfig(void) {
  //
  // Set a marker that we can evaluate in one of the CGI callbacks
  // to let the user know if he did not call this routine in his
  // application. The CGI VFile callbacks are added on another way,
  // so they will be present even without calling this routine.
  //
  _ConfigCalled = 1u;
  //
  // Set a file info callback that knows which files are virtual.
  //
  ...
  //
  IP_WEBS_AddRequestNotifyHook(&_RequestNotifyHook,        _cbRequestNotifyHook);
  ...
  //
  // Add auth error callback.
  //
  IP_WEBS_SetAuthErrorCallback(_pfAuthFailed);
  //
  // Add gzip support
  //
